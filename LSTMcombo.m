% LSTMcombo.m
% this script computes the response to all drug combinations
% LSTM networks must trianed first
...
% define LSTM unit state and net vectors
% state vectors
...
for cmb = cmbLim:cmbLim % for combo limit combinations
    % *** combo slot different for RADC and NACC ***
    % *** this is CRUCIALLY important!!! ***
    % inArr(:,11:27) = meshgrid(comboArray(cmb,:),1:numAge); % for RADC 17
    % inArr(:,21:37) = meshgrid(comboArray(cmb,:),1:numAge); % for NACC 17
    % inArr(:,11:26) = meshgrid(comboArray(cmb,:),1:numAge); % for RADC 16
    % inArr(:,21:36) = meshgrid(comboArray(cmb,:),1:numAge); % for NACC 16
    % inArr(:,11:21) = meshgrid(comboArray(cmb,:),1:numAge); % for RADC 11
    % inArr(:,21:31) = meshgrid(comboArray(cmb,:),1:numAge); % for NACC 11
    inArr(:,11:11+numDrugs-1) = meshgrid(comboArray(cmb,:),1:numAge); % RADC
    % inArr(:,21:21+numDrugs-1) = meshgrid(comboArray(cmb,:),1:numAge); % NACC
    
    % now x can simply be set equal to the input array, but transposed
    x = inArr';
...
plot(ageSerNorm',ageCogArray(1:1000:cmbLim,:))
hold
plot(ageSerNorm',ageCogArray(1,:),'r','linewidth',3)
% plot(ACTage,ACTcog,'.')
hold
% axis([0,1,0.5,0.7])
xlabel('age')
ylabel('comp cog score')
